    ...
    <div class="card-body">
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                @if($product->foto)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $product->foto) }}" alt="{{ $product->nama }}" class="rounded" width="100">
                ...
                @error('foto')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                <div class="form-text">Max size: 2MB. Allowed formats: JPEG, PNG, JPG, GIF. Leave empty to keep current photo.</div>
            </div>
